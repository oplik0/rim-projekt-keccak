        ...
        if rising_edge(ap_clk) then
            if rst = '1' then
                ...
                case fsm_state is
                    ...
                    -- Read input words, XOR into state
                    when S_ABSORB_READ =>
                        ap_idle_i <= '0';
                    ...
                    -- Apply pad10*1 with domain separator
                    -- (delimiter at offset_bytes, 0x80 at last byte of rate)
                    when S_PADDING =>
                    ...
                    -- Wait for post-padding permutation
                    when S_FINAL_PERM =>
                        if perm_done = '1' then
                            state <= perm_out;
                            squeeze_idx <= (others => '0');
                    ...
                    when S_DONE =>
                        ap_done_i <= '1';
                        ap_ready_i <= '1';
                        ap_idle_i <= '1';  -- Signal idle before returning
                        fsm_state <= S_IDLE;
