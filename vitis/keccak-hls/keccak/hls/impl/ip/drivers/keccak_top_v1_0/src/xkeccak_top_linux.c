...
typedef struct {
    ...
    XKeccak_top_uio_map maps[ MAX_UIO_MAPS ];
} XKeccak_top_uio_info;

/***************** Variable Definitions **************************************/
static XKeccak_top_uio_info uio_info;
...
static int uio_info_read_name(XKeccak_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKeccak_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKeccak_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKeccak_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKeccak_top_Initialize(XKeccak_top *InstancePtr, const char* InstanceName) {
	XKeccak_top_uio_info *InfoPtr = &uio_info;
